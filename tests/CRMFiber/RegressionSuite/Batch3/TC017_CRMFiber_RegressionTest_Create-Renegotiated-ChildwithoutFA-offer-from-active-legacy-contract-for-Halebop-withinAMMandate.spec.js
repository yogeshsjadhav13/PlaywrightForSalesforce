...
const TestCaseName = 'TC017_CRMFiber_RegressionTest';



test('TC017_CRMFiber_RegressionTest_Create-Renegotiated-ChildwithoutFA-offer-from-active-legacy-contract-for-Halebop-withinAMMandate', async function ({ browser }) {
  ...
  var ContractName = LocalTestData.get("OpportunityName") + "_" + Math.floor(Math.random() * 90000 + 10000);
  LocalTestData.set("OpportunityName", ContractName);
  await contractPage.CreateCRMFiberStandaloneContract(LocalTestData, utilityFunctionLocal);



  var ActiveOpportunityName = ContractName;
  var FAOpportunityName = await utilityFunctionLocal.RunSOQLQuery("select Name from Opportunity where Account_Org_Nr__c = '" + LocalTestData.get("RenegotiationOrgNumber") + "' and Name like '%Playwright%'");
  ...
  LocalTestData.set("Varumarke", "Telia KO");
  ...
  LocalTestData.set("SalesFlowType", "NewSales");
  await faCartPage.AddProductToTheCart(LocalTestData);
  if (Boolean(LocalTestData.get("NewProductsToAddDuringRenegotiation"))) {
    var NewProductList = LocalTestData.get("Products") + ";" + LocalTestData.get("NewProductsToAddDuringRenegotiation");
    LocalTestData.set("Products", LocalTestData.get("NewProductsToAddDuringRenegotiation"));
    await faCartPage.AddProductToTheCart(LocalTestData);
    LocalTestData.set("Products", NewProductList);
  }
