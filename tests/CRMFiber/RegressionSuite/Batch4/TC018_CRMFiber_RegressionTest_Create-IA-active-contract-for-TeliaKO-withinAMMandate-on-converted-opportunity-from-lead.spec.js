...
const TestCaseName = 'TC018_CRMFiber_RegressionTest';



test('TC018_CRMFiber_RegressionTest_Create-IA-active-contract-for-TeliaKO-withinAMMandate-on-converted-opportunity-from-lead', async function ({ browser }) {
  ...
  var Lead = await opportunityPage.createCRMFiberLead(LocalTestData, utilityFunctionLocal);
  var LeadName = Lead[0];
  var LeadID = Lead[1];
  LocalTestData.set("LeadName",LeadName);
  LocalTestData.set("LeadID",LeadID);



  //Step 3 - Login as SOHO Ds Sales rep
  await loginPage.loginAsUser(utilityFunctionLocal, LocalTestData.get("SalesRepUser"), "C&SB");
  await opportunityPage.convertCRMFiberLeadIntoOpportunity(LocalTestData, utilityFunctionLocal);
  var OpportunityName = LocalTestData.get("OpportunityName");
