...
test('TC002_CRMFiber_RegressionTest_Create-IA-active-contract-for-TeliaKOSkanova-withinSMMandate-with-TeliaMDUSDUKundagtNat-XLAN-document-and-Terminate', async function ({ browser }) {
  ...
  const Opportunity = await opportunityPage.createCRMFiberOpportunity(LocalTestData, utilityFunctionLocal);
  ...
  //Step 13 - Perform contract termination
  await contractPage.PerformMCRMFiberContractTermination(LocalTestData, utilityFunctionLocal);
  await contractPage.PerformMCRMFiberContractCancelTermination();
  await contractPage.PerformMCRMFiberContractTermination(LocalTestData, utilityFunctionLocal);
  await utilityFunctionLocal.executeApexCode(pageAdmin, `Fiber_Automatic_Extension_Contracts  p= New Fiber_Automatic_Extension_Contracts (); database.executeBatch(p,50);`)
  await page.waitForTimeout(20000);
  await contractPage.verifyCRMFiberContractTerminationStatus(utilityFunctionLocal, OpportunityName);
