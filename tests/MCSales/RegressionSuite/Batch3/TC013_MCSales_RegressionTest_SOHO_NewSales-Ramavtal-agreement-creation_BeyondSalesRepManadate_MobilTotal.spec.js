...
const TestCaseName = 'TC013_MCSales_RegressionTest';



test('TC013_MCSales_RegressionTest_SOHO_NewSales-Ramavtal-agreement-creation_BeyondSalesRepManadate_MobilTotal', async function ({ browser }) {
  ...
  await page.reload();
  //LTAART-321 - Negotiate minute based fees workaround
  await page.getByRole('button', { name: ' Ändra prisförhandling' }).click();
  //await page.locator("//button[contains(normalize-space(.),'Show menu')]").click();
  //await page.locator("//div[contains(@class,'action')]//*[contains(text(),'Ändra prisförhandling')]").first().click();
  ...
  await page.waitForTimeout(2000);
  await page.reload();
  LocalTestData.set("Products", "Försäkring Mobiltelefon");
  await faCartPage.AddProductToTheCart(LocalTestData);
  await faCartPage.ConfigureMCSalesProductInTheCart(LocalTestData);
  await faCartPage.PerformValideraPriserAndVerifyApprovalFlags(LocalTestData);
  await faCartPage.cartStangKonfiguration.click();
  ...
  const secretsData = await utilityFunctionLocal.fetchEnvironmentCreds();
  await page.goto(secretsData.get("environmentURL") + "/lightning/r/Account/" + ContractID + "/view");
  await page.getByRole('button', { name: ' Ändra prisförhandling' }).click();
  await expect(page.locator('#modal-content-id-1')).toContainText('Är du säker på att du vill ändra prisförhandlingen? Klicka på Fortsätt för att komma till varukorgen.');
  await page.getByText('Fortsätt', { exact: true }).click();
  await page.waitForTimeout(2000);
  await page.reload();
  LocalTestData.set("Products", "Smart Säkerhet");
  await faCartPage.AddProductToTheCart(LocalTestData);
  await faCartPage.ConfigureMCSalesProductInTheCart(LocalTestData);
  await faCartPage.PerformValideraPriserAndVerifyApprovalFlags(LocalTestData);
  await faCartPage.PerformSlutforKonfigurationAndQuoteApproval(LocalTestData, OpportunityName);
  ...
  await page.getByText('Fortsätt', { exact: true }).click();
  await faCartPage.cartSkapaOffertdokumentButton.click();
  page.on('dialog', dialog => dialog.accept("Vid ändringar i pris/ produkt så kommer ditt tidigare avtal att avbrytas och ett nytt avtal måste skapas när offerten är godkänd av kund."));
  await quotePage.nextSwedishButton.click();
  await quotePage.nextSwedishButton.click();
  await quotePage.nextSwedishButton.click();
  await quotePage.checkaInButton.click();
  ...
  const TUPPInterfacetStatus = await utilityFunctionLocal.RunSOQLQuery("select TeliaSE_TUPP_Interface_Name__c from Contract where id = '" + ContractID + "'");
  if (TUPPInterfacetStatus !== "DeleteAgreementCase") {
    expect(true).toBe(false);
  }


  LocalTestData.set("Products", "Mobilupplägg TOTAL;Försäkring Mobiltelefon;Smart Säkerhet");
  await quotePage.performSkapaAvtalOnMCSalesQuote(LocalTestData, utilityFunctionLocal);
  const QuoteID = await utilityFunctionLocal.RunSOQLQuery("select id from Quote where Type_of_Quote__c = 'Prisförhandling' and OpportunityId = '" + OpportunityID + "'");
  await page.goto(secretsData.get("environmentURL") + "/lightning/r/Account/" + QuoteID + "/view");
  await quotePage.performSkapaOffertdokumentOnMCSalesQuote(LocalTestData, OpportunityName);
