...
test('TC015_MCSales_RegressionTest_LARGE_NewSales-Ramavtal-agreement-creation_BeyondSalesRepManadate_MobilTotal-BBStart-Datanet-CloudVPNLan-TP', async function ({ browser }) {
  ...
  await expect(faCartPage.cartSkapaOffertDokumentErrorMessage).toContainText('Sekundär access finns med i varukorgen, för SLA-D krävs en Hera förfrågan.');
  ...
  await page.locator('button').filter({ hasText: 'Orderunderlag CO' }).click();
  await page.locator("//span[text()='Fortsätt för att skapa orderunderlag']").click();
  await page.locator("//button[text()='Fortsätt']").click();
