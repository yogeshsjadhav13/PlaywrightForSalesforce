...
const TestCaseName = 'TC046_MCSales_RegressionTest';



test('TC046_MCSales_RegressionTest_SOHO_Inforhandling-Ramavtal-agreement-creation_BeyondSalesRepManadate_MobilPlus-SC', async function ({ browser }) {
  ...
  await page.frameLocator('iframe[name*="vfFrameId"]').first().getByPlaceholder('Sök').fill("mobil");
  ...
  expect(await page.frameLocator('iframe[name*="vfFrameId"]').first().locator("//p[normalize-space(.)='Mobilupplägg TOTAL' or normalize-space(.)='Mobilupplägg All-IN+' or normalize-space(.)='SLUTANVÄNDARSUPPORT – 1:A LINJE MOBILT' or normalize-space(.)='SLUTANVÄNDARSUPPORT – 2:A LINJE MOBILT']//..//..//..//button[normalize-space(.)='Lägg till']").count()).toEqual(0);



  //Step 6 - Add Products to the cart and configure withing sales rep mandate
  await faCartPage.ConfigureMCSalesProductForNegotiationInTheCart(LocalTestData);
  ...
  for (let iteration = 0; iteration < Products.length; iteration++) {
    if (Products[iteration] === "Mobilupplägg TOTAL" || Products[iteration] === "Mobilupplägg All-IN+") {
      LocalTestData.set("Products", Products[iteration]);
      break;
    }
  }
  ...
  await expect(quotePage.skapaAvtalMCSalesFlowIdentifierText).toContainText('Inforhandling');
  await expect(quotePage.skapaAvtalTUPPInterfaceNameText).toContainText('AddItemsToAgreement');


  //Step 11 - Perform skapa avtalsdokument flow
