...
const csvFileName = 'UtilizerAccounts_TC037';


test('TC037_MCSales_RegressionTest_SOHO_NewSales-Ramavtal-agreement-creation-with-mutiple-utilizers_MobilTotal-O365-ITSupport', async function ({ browser }) {
  ...
  LocalTestData.set("Products", "Microsoft 365");
  LocalTestData.set("Microsoft365SubProducts", "Microsoft 365 Business Basic EEA (no Teams)");
  const ProductName = "Microsoft 365";
/*  
  ...
  await faCartPage.configureBindingstidPicklist.selectOption("12 months");
  await faCartPage.configureSparaSavingButton.click();
  await faCartPage.cartFrame.locator("//h2[contains(text(),'Uppdaterad tjänst: " + ProductName + "')]").click();
  await faCartPage.configureCloseButton.click();
*/

  LocalTestData.set("Microsoft365SubProducts", "Microsoft 365 Business Standard EEA (no Teams)");
  await faCartPage.AddProductToTheCart(LocalTestData);
  await page.waitForTimeout(20000);
  await faCartPage.ConfigureMCSalesProductInTheCart(LocalTestData);
  await faCartPage.cartFrame.locator("//span[text()='" + ProductName + "']//ancestor::div[@class='cpq-product-cart-alignment']//a[contains(@ng-click,'config')]").first().click({ timeout: 5000 });
  await faCartPage.configureBindingstidPicklist.selectOption("1 month");
  await faCartPage.configureSparaSavingButton.click();
  await faCartPage.cartFrame.locator("//h2[contains(text(),'Uppdaterad tjänst: " + ProductName + "')]").click();
  await faCartPage.configureCloseButton.click();
  ...
  LocalTestData = await utilityFunctionLocal.ReadDataFromExcel();
  await faCartPage.AddProductToTheCart(LocalTestData);
  ...
  await expect(page.locator("//vlocity_cmt-omniscript-text-block[@data-omni-key='errorMessage_FileUpload']")).toContainText('Felmeddelanden');
  await expect(page.locator("//vlocity_cmt-omniscript-text-block[@data-omni-key='errorMessage_FileUpload']")).toContainText('Det uppstod problem med att lägga till några av nyttjarna. Klicka här för att ladda se de drabbade nyttjarna och orsaken.');
  await quotePage.nextSwedishButton.click();
  ...
  await expect(page.getByRole('article', { name: 'Anteckningar och bilagor' })).toContainText('Failed Utilizers_');
  await expect(page.getByLabel('Anteckningar och bilagor').getByRole('list')).toContainText(csvFileName);
  //await page.locator('li').filter({ hasText: /Failed Utilizers/ }).first().hover();
  //await page.locator('li').filter({ hasText: /UtilizerAccounts_TC037/ }).first().hover();
  ...
  //Step 12 - Perform prisforhandling, update only quanitity for Mobil product, and get the approved quote
  await page.getByRole('button', { name: ' Ändra prisförhandling' }).click();
  await page.getByText('Fortsätt', { exact: true }).click();
  await page.waitForTimeout(5000);
  LocalTestData.set("Products", "IT-support Plus");
  LocalTestData.set("ProductOfferQuantity", "5");
  await page.reload();
  await faCartPage.AddProductToTheCart(LocalTestData);
  await faCartPage.ConfigureMCSalesProductInTheCart(LocalTestData);
  await faCartPage.PerformValideraPriserAndVerifyApprovalFlags(LocalTestData);
  ...
  await expect(faCartPage.cartSkapaOffertDokumentErrorMessage).toContainText('Det finns offertkvantitet på flera av erbjudandena nedan, vänligen ändra så det bara finns offertkvantitet på ett av erbjudandena nedan innan du skapar offert. IT-avdelning, IT-avdelning Start, IT-support Standard, IT-support Plus.');
  await faCartPage.cartSkapaOffertDokumentErrorMessageCloseButton.click();
  await page.reload();
  LocalTestData.set("ProductOfferQuantity", "");
  await faCartPage.ConfigureMCSalesProductInTheCart(LocalTestData);
  await faCartPage.PerformValideraPriserAndVerifyApprovalFlags(LocalTestData);
  await faCartPage.PerformSlutforKonfigurationAndQuoteApproval(LocalTestData, OpportunityName);
  await quotePage.skapaAvtalButton.click();
  await expect(quotePage.cartFrame.getByRole('paragraph')).toContainText('I varukorgen finns flera erbjudanden från samma familj, behåll bara ett av erbjudandena nedan för att skapa ditt kontrakt:');
  await expect(quotePage.cartFrame.getByRole('paragraph')).toContainText('IT-avdelningIT-avdelning StartIT-support StandardIT-support Plus');
