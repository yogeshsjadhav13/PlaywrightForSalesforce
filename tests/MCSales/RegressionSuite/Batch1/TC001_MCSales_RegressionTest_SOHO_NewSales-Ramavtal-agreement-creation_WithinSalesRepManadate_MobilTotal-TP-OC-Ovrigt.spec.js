...
test('TC001_MCSales_RegressionTest_SOHO_NewSales-Ramavtal-agreement-creation_WithinSalesRepManadate_MobilTotal-TP-OC-Ovrigt', async function ({ browser }) {
  ...
  await faCartPage.OperatorConnectNoMobilErrorMessage1.click();
  await faCartPage.OperatorConnectNoMobilErrorMessage2.click();
  await faCartPage.OperatorConnectNoMobilErrorMessageCloseButton.click();
  await faCartPage.DeleteSingleProductToTheCart(ProductName);


  //Verify error message for Smart Connect
  ProductName = "Smart Connect";
  await faCartPage.AddSingleProductToTheCart(ProductName);
  await faCartPage.PerformProductConfiguration(LocalTestData, ProductName);
  await faCartPage.cartValideraVarukorgenButton.click();
  await faCartPage.SmartConnectNoMobilErrorMessage.click();
  await faCartPage.SmartConnectNoMobilErrorMessageCloseButton.click();
  await faCartPage.cartSlutforKonfiguration.click();
  await faCartPage.SmartConnectNoMobilErrorMessage.click();
  await faCartPage.SmartConnectNoMobilErrorMessageCloseButton.click();
  await faCartPage.cartSkapaOffertdokumentButton.click();
  await faCartPage.SmartConnectNoMobilErrorMessage.click();
  await faCartPage.SmartConnectNoMobilErrorMessageCloseButton.click();
  await faCartPage.DeleteSingleProductToTheCart(ProductName);
  ...
  for (let iteration = 0; iteration < Products.length; iteration++) {
    if (Products[iteration] === "Mobilupplägg TOTAL" || Products[iteration] === "Mobilupplägg All-IN+") {
      await faCartPage.cartFrame.locator("//div[@class='cpq-product-cart-item' and contains(normalize-space(.),'" + Products[iteration] + "')]//child::span[text()='Engångs-/Öppningsavgift']").hover();
    } else {
      await faCartPage.cartFrame.locator("//div[@class='cpq-product-cart-item' and contains(normalize-space(.),'" + Products[iteration] + "')]//child::span[text()='Engångsavgift']").hover();
    }
  }
  if (LocalTestData.get("SalesFlowType") === "Renegotiation") {
  ...
  //Step 6 - Verify price validation error message on cart operations
  await faCartPage.cartSkapaOffertdokumentButton.click();
  await page.waitForTimeout(1000);
  await expect(faCartPage.cartSkapaOffertDokumentErrorMessage).toContainText('Validera varukorgen innan du fortsätter');
  ...
  await expect(faCartPage.cartSkapaOffertDokumentErrorMessage).toContainText('Validera varukorgen innan du fortsätter');
  await faCartPage.cartSkapaOffertDokumentErrorMessageCloseButton.click();
  await faCartPage.cartFrame.locator("//span[normalize-space(.)='Mobilupplägg TOTAL']//ancestor::div[@class='cpq-product-cart-item']//div[@class='cpq-item-no-children' and text()='Jobbmobil 1 GB']//ancestor::div[@class='cpq-item-base-product']//button[contains(text(),'Lägg till')]").first().click();
  await page.waitForTimeout(5000);
  await expect(faCartPage.cartFrame.locator("//div[@class='slds-col slds-align-middle']/h2[@ng-if='title']")).toContainText('Jobbmobil 1 GB');
