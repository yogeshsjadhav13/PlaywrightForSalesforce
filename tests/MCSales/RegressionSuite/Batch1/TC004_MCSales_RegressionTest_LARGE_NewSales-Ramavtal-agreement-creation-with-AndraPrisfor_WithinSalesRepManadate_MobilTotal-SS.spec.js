...
const TestCaseName = 'TC004_MCSales_RegressionTest';



test('TC004_MCSales_RegressionTest_LARGE_NewSales-Ramavtal-agreement-creation-with-AndraPrisforhandling_WithinSalesRepManadate_MobilTotal-SmartSec', async function ({ browser }) {
  ...
  //Step 11 - Send the contract for signing
  ...
  await page.getByRole('button', { name: 'Markera Status som färdig(t)' }).click();
  await page.locator("//div[contains(@class,'active') and contains(@class,'window')]//span[text()='Status']//..//..//..//lightning-formatted-text[text()='Avslaget']").click();
  ...
  LocalTestData.set("DiscountPercentage", 0.03);
  LocalTestData.set("ProductOfferQuantity", 7);
  LocalTestData.set("UtilizerAccountOrgNumber", "");
  await page.reload();
  await faCartPage.ConfigureMCSalesProductForRenegotiationInTheCart(LocalTestData);
  await faCartPage.PerformValideraPriserAndVerifyApprovalFlags(LocalTestData);
  await faCartPage.PerformSlutforKonfigurationAndQuoteApproval(LocalTestData, OpportunityName);
  ...
  //Step 14 - Perform Skapa offertdokument, contract creation and contract document creation
  await quotePage.performSkapaOffertdokumentOnMCSalesQuote(LocalTestData, OpportunityName);
