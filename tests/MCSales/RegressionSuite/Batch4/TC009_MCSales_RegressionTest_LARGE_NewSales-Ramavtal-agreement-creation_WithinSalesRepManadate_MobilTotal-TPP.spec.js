...
test('TC009_MCSales_RegressionTest_LARGE_NewSales-Ramavtal-agreement-creation_WithinSalesRepManadate_MobilTotal-TPP', async function ({ browser }) {
  ...
  //Step 10 - Sign the contract manually
  ...
  //B2XSET-309626 - Disabled due to defect
  //Step 11 - Download the orderunderlag for TPP and verify Se Status for e-sign functionality
  const downloadPromise = page.waitForEvent('download');
  await page.locator('button').filter({ hasText: 'Orderunderlag TPP' }).click();
  const download = await downloadPromise;
  await page.getByRole('tab', { name: 'Relaterat' }).click();
  await page.locator("//span[@title='Anteckningar och bilagor']//parent::a").first().click();
  await page.locator("//table[@aria-label='Anteckningar och bilagor']//span[contains(@title,'Pris- och beställningsunderlag Touchpoint Plus')]").click();
  await page.getByRole('button', { name: 'Stäng' }).click();
  await page.goBack();
  const pagePromise = context.waitForEvent('page');
  await page.locator('button').filter({ hasText: 'Se status för e-sign' }).click();
  const newPage = await pagePromise;
  await newPage.waitForLoadState();
  await newPage.frameLocator('//div[contains(@class,"normal")]//iframe[contains(@name,"vfFrameId")]').getByRole('cell', { name: 'Sent' }).click();
