...
const TestCaseName = 'TC022_MCSales_RegressionTest';



test('TC022_MCSales_RegressionTest_SOHO_Renegotiation-Ramavtal-agreement-creation_BeyondSalesRepManadate_MobilTotal-SC', async function ({ browser }) {
  ...
  await quotePage.skapaAvtalAddUtilizerNejCheckbox.click();
  await quotePage.nextButtonInsideFrame.click();
...
/*
  ...
  var status = await utilityFunctionLocal.RunSOQLQuery("select TeliaSE_TUPP_Interface_Name__c from contract where vlocity_cmt__OpportunityId__c = '" + OpportunityID + "'");
  for (let i = 1; i <= 60; i++) {
    status = await utilityFunctionLocal.RunSOQLQuery("select TeliaSE_TUPP_Interface_Name__c from contract where vlocity_cmt__OpportunityId__c = '" + OpportunityID + "'");
    if (status === "TUPPUpdateService") {
      var TuppStatus = await utilityFunctionLocal.RunSOQLQuery("select TeliaSE_TUPP_Status__c from contract where vlocity_cmt__OpportunityId__c = '" + OpportunityID + "'");
      console.log("Tupp status is - " + TuppStatus);
      if (TuppStatus !== "Active") {
        expect(true).toBe(false);
      }
      await page.reload();
      await contractPage.contractRedigerButton.click();
      await contractPage.contractCancellationReason.type("Test Automation Cancel");
      await contractPage.contractStatusPicklist1.click();
      await page.getByRole('option', { name: 'Avbruten' }).getByText('Avbruten').click();
      await contractPage.contractSparaButton.click();
      await contractPage.contractAvbrutenStatus.click();
      break;
    }
